...
if __name__ == '__main__':
    ...
    print('90')
    segment_size = 32  # 32 is by default and it is very short (only 32 symbols)!
    dropout = 0.3
    epochs_top = 1
    iterations_top = 2
    batch_size_top = 1024
    learning_rate_top = 1e-5  # The learning rate we used in the paper was 1e-4. However, if you are doing additional steps of pre-training starting from an existing BERT checkpoint, you should use a smaller learning rate (e.g., 2e-5). More  https://github.com/google-research/bert#pre-training-tips-and-caveats
    epochs_all = 1  # by default 4
    iterations_all = 1  # by default 3
    batch_size_all = 90  #50  # 256
    learning_rate_all = 1e-5  # see comment for learning_rate_top
    hyperparameters = {
    ...
    with open("train2012_MAX.txt", 'r', encoding="utf8", errors="ignore") as f:  # encoding='cp1252', encoding='utf8', errors="replace"
    ...
    bert_punc, optimizer, best_val_loss = train(bert_punc, optimizer, criterion, epochs_all,
        data_loader_train, data_loader_valid, save_path, punctuation_enc, iterations_all, best_val_loss=1e9)
    print('Done!!!')
